...
class BlockyNoteBlockListener : Listener {
    ...
    fun BlockDamageEvent.onDamageVanillaBlock() {
        ...
        val mining =
            PlayerMiningAttribute(NoteBlockHelpers.vanillaBreakingComponent.createBreakingModifier(player, block))
    ...
    fun PlayerInteractEvent.onPlaceAgainstBlockyBlock() {
        val (placedAgainst, item, hand) = (clickedBlock?.takeIf { it.isBlockyBlock }
            ?: return) to (item?.takeIf { it.type.isBlock } ?: return) to (hand ?: return)
    ...
    fun PlayerInteractEvent.onPrePlacingBlockyNoteBlock() {
        ...
        val gearyItem =
            player.gearyInventory?.get(hand)?.takeIf { it.get<SetBlock>()?.blockType == SetBlock.BlockType.NOTEBLOCK }
                ?: return
    ...
    fun BlockPlaceEvent.onPlaceNoteBlock() {
        if (blockPlaced.isVanillaNoteBlock) blockPlaced.container { encode(VanillaNoteBlock()) }
    }

    @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true)
    fun ChunkLoadEvent.migrateOnChunkLoad() {
        CustomBlockData.getBlocksWithCustomData(blocky.plugin, chunk)
            .filter { it.blockData is NoteBlock && it.container { has<VanillaNoteBlock>() } }
            .forEach { block ->
                ...
                if (block.customBlockData.isEmpty) {
                    block.container { encode(VanillaNoteBlock(0)) }
                    ...
                    block.container { encode(VanillaNoteBlock((block.blockData as NoteBlock).note.id.toInt())) }
                    block.blockData = Material.NOTE_BLOCK.createBlockData()
                }
            }
    }
